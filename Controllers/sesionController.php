<?php 
    ...
    if (array_key_exists('id', $_GET)) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
            try {
                $query = $connection->prepare('DELETE FROM sesion WHERE id = :id AND token_acceso = :token_acceso');
        ...
        elseif($_SERVER['REQUEST_METHOD'] === 'PATCH') {
            ...
            try{
                ...
                $query = $connection->prepare('SELECT sesion.id AS id_sesion, sesion.id_usuario, activo, 
                token_acceso, token_actualizacion, caducidad_token_acceso, caducidad_token_actualizacion 
                FROM sesiones, usuarios WHERE sesion.id_usuario = usuarios.id AND sesion.id = :id_sesion 
                ...
                $query = $connection->prepare('UPDATE sesiones SET token_acceso = :token_acceso, caducidad_token_acceso = DATE_ADD(NOW(), 
                INTERVAL :caducidad_tacceso_s SECOND), token_actualizacion = :token_actualizacion, caducidad_token_actualizacion = DATE_ADD(NOW(), 
                INTERVAL :caducidad_tactualizacion_s SECOND) WHERE id = :id_sesion AND id_usuario = :id_usuario AND token_acceso = :consulta_tokenAcceso 
                AND token_actualizacion = :consulta_tokenActualizacion');
    ...
    elseif (empty($_GET)) {
        ...
        if (!isset($jsonData->email) || !isset($jsonData->contrasena)) {
            $response = new Response();
            $response->setHttpStatusCode(400);
            $response->setSuccess(false);
            (!isset($jsonData->email) ? $response->addMessage("El email es obligatorio") : false);
        ...
        try {
            ...
            $query = $connection->prepare('SELECT id, rol, email, contrasena, nombre, activo FROM usuario WHERE email = :email');
        ...
        try{
            ...
            $query = $connection->prepare('INSERT INTO sesion(id_usuario, token_acceso, caducidad_token_acceso, token_actualizacion, caducidad_token_actualizacion) 
            VALUES (:id_usuario, :token_acceso, DATE_ADD(NOW(), INTERVAL :caducidad_tacceso_s SECOND), :token_actualizacion, 
            DATE_ADD(NOW(), INTERVAL :caducidad_tactualizacion_s SECOND))');
