...
if(empty($_GET)) {
    if($_SERVER['REQUEST_METHOD'] === 'POST')
    {
        ...
        try {
            ...
            $query = $connection->prepare('INSERT INTO compra_producto (id_compra, id_producto) 
            VALUES (:precio, :id_usuario )');
            $query->bindParam(':id_usuario', $id_usuario, PDO::PARAM_INT);
            $query->bindParam(':precio', $json_data->id_compra, PDO::PARAM_INT);
            $query->execute();
...
if (array_key_exists("compraprd", $_GET)) {
    //GET host/tareas/categoria_id={id}
    if($_SERVER['REQUEST_METHOD'] === 'GET')
    {
        //Devolver por categoria
        $categoria_id = $_GET['compraprd'];
        if ($categoria_id == '' || !is_numeric($categoria_id)) {
            $response = new Response();
            $response->setHttpStatusCode(400);
            $response->setSuccess(false);
            $response->addMessage("El id de compra no puede estar vacío y debe ser numérico");
            $response->send();
            exit();
        }

        try {
            $query = $connection->prepare('SELECT id_producto FROM compra_producto WHERE id_compra = :categoria_id');
            ...
            while($row = $query->fetch(PDO::FETCH_ASSOC)) {
                ...
                $tareas[] = $row['id_producto'];
            }

            $returnData = array();
            $returnData['total_registros'] = $rowCount;
            $returnData['productos'] = $tareas;
            ...
            $response->setHttpStatusCode(500);
            $response->setSuccess(false);
            $response->addMessage("Error en consulta de los productos");
            $response->send();
            exit();
        }
    }
